...
## 🚀 Relevance to Automotive Systems
In automotive modules such as Body Control or Memory Seats, microcontrollers cannot waste clock cycles polling for button presses. They must utilize external interrupts to wake up from sleep states or immediately alter actuator trajectories. This project mimics that exact asynchronous requirement, ensuring zero missed inputs regardless of the CPU's current workload.
